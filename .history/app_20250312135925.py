...
def format_justification(justification):
    ...
    # Ensure numbered lists are preserved by replacing "1. ", "2. ", etc., with <br> for proper formatting
    justification = re.sub(r'(\d+\.) ', r'<br>\1 ', justification)

    # Convert newline characters into HTML line breaks
    justification = justification.replace('\n', '<br>')

    return Markup(justification)  # Markup ensures it renders properly in Jinja templates
        ...
        if line.strip():
            if line.startswith('Criterion:'):
                ...
                if len(parts) >= 3:
                    ...
                    # Process the justification to handle formatting better
                    ...
                    criteria_results.append({
                        'name': criterion_name,
                        'grade': grade,
                        'justification': Markup(justification)  # Use Markup to render HTML safely
                    })
    
    # Debug print to verify data being sent to the template
