...
from g4f.errors import ResponseStatusError  # Import g4f specific errors
...
async def retry_request(func, max_retries=3):
    """Retries an API request with exponential backoff in case of errors."""
    loop = asyncio.get_event_loop()

    for i in range(max_retries):
        try:
            return await loop.run_in_executor(None, func)  # Run sync function in executor
        except OpenAIError as e:  
            if "rate limit" in str(e).lower():  # Handle rate limits
...
async def grade_essay(essay_text, context_text):
    ...
    for criterion in criteria:
        ...
        response = await retry_request(lambda: client.chat.completions.create(
            model="gpt-4o",
            messages=[{
                "role": "user",
                "content": (
...
def process_essay():
    ...
    summary_result = generate_summary(original_text)  # Assuming this is synchronous
    grade_result = asyncio.run(grade_essay(original_text, context_text))  # Force sync execution
