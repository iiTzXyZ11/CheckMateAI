...
async def retry_request(func, max_retries=3):
    """Retries an API request with exponential backoff in case of rate limits."""
    loop = asyncio.get_event_loop()

    for i in range(max_retries):
        try:
            return await loop.run_in_executor(None, func)  # Run in background
        except Exception as e:  
...
def process_essay():
    ...
    loop = None  # Ensure loop is defined before try-finally
    try:
        loop = asyncio.new_event_loop()
        asyncio.set_event_loop(loop)
        grade_result = loop.run_until_complete(grade_essay(original_text, context_text))
    finally:
        if loop is not None:
            loop.close()  # Prevents unbound variable error
