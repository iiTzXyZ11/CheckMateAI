    ...
    try:
        for criterion in criteria:
            ...
            if not hasattr(response, 'choices') or len(response.choices) == 0:
                grades_per_criterion.append(f"Criterion: {criterion['name']} - Error: No response received")
                continue
            ...
            grades_per_criterion.append(f"Criterion: {criterion['name']} - Grade: {points_received}/{criterion['points_possible']}\nJustification: {justification}")

        # Format the final output with a clear structure
        final_output = f"Final Grade: {total_points_received}/{total_points_possible}\n\n"
        final_output += "\n".join(grades_per_criterion)
        
        print("Final grading output:", final_output)  # Debugging output
        return final_output

    except Exception as e:
        print(f"Error in grade_essay: {str(e)}")
        return f"Error occurred during grading: {str(e)}"
...
def process_essay():
    try:
        ...
        print("Summary Result:", summary_result)  # Debug output
        print("Grade Result:", grade_result)      # Debug output
        ...
        print(f"Error in process_essay: {str(e)}")
